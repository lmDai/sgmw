...
import com.uiho.module_palm.ui.adapter.TravelDaysClassifyAdapter;
...
@Route(path = RouterPath.PALM_TRAVEL_INFO_MONTH) // 路由地址，必须注明
public class TravelInfoClassifyMonthFragment extends BaseMvpFragment<TravelInfoClassifyContract.View, TravelInfoClassifyPresenter> implements TravelInfoClassifyContract.View {
    ...
    TravelDaysClassifyAdapter adapter;
    ...
    protected void initView(LayoutInflater inflater) {
        try {
            ...
            startDate = DateUtils.getMonthFirst(DateUtils.getPreDayString());
        ...
        adapter = new TravelDaysClassifyAdapter(mList);
...
//        });
    ...
    public void click(boolean isNext) {
        ...
        try {
            if (isNext) {//是否为下一个月
                str = DateUtils.getNextDay(endDate, 1);
            } else {
                str = DateUtils.getNextDay(startDate, -1);
            }
            endDate = DateUtils.getMonthLast(str);
            startDate = DateUtils.getMonthFirst(str);
    ...
    public static TravelInfoClassifyMonthFragment getInstance(String vin) {
        TravelInfoClassifyMonthFragment fragment = new TravelInfoClassifyMonthFragment();
