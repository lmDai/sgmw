...
//    private int totalMovedLength;
    public NumberProgressBar(Context context, AttributeSet attrs) {
        ...
        // 构造器中初始化数据
        smallR = SystemUtils.dp2px(context, 4);//小圆半径
        bigR = SystemUtils.dp2px(context, 8);//大圆半径
        radius = SystemUtils.dp2px(context, 10) / 2;//进度条高度
        jR = SystemUtils.dp2px(context, 6);//矩形
    ...
    private void initData() {
        ...
        paintStart.setStrokeWidth(SystemUtils.dp2px(context, 1));
        paintStart.setDither(true);
        paintStart.setAntiAlias(true);
        paintStart.setStyle(Paint.Style.FILL);

        // 已完成进度条画笔的属性
        paintInit.setColor(context.getResources().getColor(R.color.color_bule));
        paintInit.setStrokeWidth(SystemUtils.dp2px(context, 1));
        ...
        // 大圆画笔
        paintEndBig.setColor(context.getResources().getColor(R.color.color_bule));
        paintEndBig.setAntiAlias(true);
        paintEndBig.setStyle(Paint.Style.FILL);


        // 百分比文字画笔的属性
        int paintTextSizePx = SystemUtils.sp2px(context, 11);  //设置百分比文字的尺寸
        paintText.setColor(context.getResources().getColor(R.color.color_bule));
